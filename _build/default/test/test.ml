...
let player_test_buy_city
    (name : string)
    (pl : Player.t)
    (expected_output : Player.t) =
  name >:: fun _ -> assert_equal expected_output (buy_city pl)

let player_test_buy_settlement
    (name : string)
    (pl : Player.t)
    (expected_output : Player.t) =
  name >:: fun _ -> assert_equal expected_output (buy_settlement pl)

let player_test_buy_road
    (name : string)
    (pl : Player.t)
    (expected_output : Player.t) =
  name >:: fun _ -> assert_equal expected_output (Player.buy_road pl)

let player_test_trade_resource_with_port
    (name : string)
    (r1 : resource)
    (r2 : resource)
    (pl : Player.t)
  ...
  assert_equal expected_output
    (Player.trade_resource_with_port pl r1 r2)

let player_test_with_new_resource
    (name : string)
    (r1 : resource)
    (pl : Player.t)
    (expected_output : Player.t) =
  name >:: fun _ ->
  assert_equal expected_output (Player.player_with_new_resource pl r1)

(*new player city tests*)
...
let player_after_port =
  create_player "123456"
    [ (Brick, 5); (Ore, 3); (Wool, 2); (Grain, 1); (Lumber, 4) ]
    1 ANSITerminal.red

let player_buyer =
  create_player "123456"
    [ (Brick, 5); (Ore, 5); (Wool, 5); (Grain, 5); (Lumber, 5) ]
    1 ANSITerminal.red

let player_buyer_more_ore =
  create_player "123456"
    [ (Brick, 5); (Ore, 6); (Wool, 5); (Grain, 5); (Lumber, 5) ]
    1 ANSITerminal.red

let player_buy_city =
  create_player "123456"
    [ (Brick, 5); (Ore, 2); (Wool, 5); (Grain, 3); (Lumber, 5) ]
    1 ANSITerminal.red

let player_buy_settlement =
  create_player "123456"
    [ (Brick, 4); (Ore, 5); (Wool, 4); (Grain, 4); (Lumber, 4) ]
    1 ANSITerminal.red

let player_buy_road =
  create_player "123456"
    [ (Brick, 4); (Ore, 5); (Wool, 5); (Grain, 5); (Lumber, 4) ]
    1 ANSITerminal.red
...
let player_tests =
  [
    player_test_with_new_resource "Test player with new resource basic"
      Ore player_buyer player_buyer_more_ore;
    player_test_trade_resource_with_port
      "Test player trade resource with port ore wool" Wool Ore
      player_with_more_resources player_after_port;
    player_test_buy_city "Test player buys city basic" player_buyer
      player_buy_city;
    player_test_buy_settlement "Test player buys settlement basic"
      player_buyer player_buy_settlement;
    player_test_buy_road "Test player buys road basic" player_buyer
      player_buy_road;
